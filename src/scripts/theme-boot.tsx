...
export function ThemeBootScript() {
  ...
  const validThemePresets = JSON.stringify(THEME_PRESET_VALUES);

  // Migration map for renamed themes
  const themeMigrations = JSON.stringify({
    "theme-mbos-new": "theme-mbos-v1",
  });
  ...
  const code = `
    (function () {
      try {
        ...
        var VALID_THEME_PRESETS = ${validThemePresets};
        var THEME_MIGRATIONS = ${themeMigrations};
        ...
        // Migrate old theme preset values to new ones
        var migratedPreset = THEME_MIGRATIONS[rawPreset] || rawPreset;
        // Validate theme preset
        var isValidPreset = VALID_THEME_PRESETS.includes(migratedPreset);
        var preset = isValidPreset ? migratedPreset : DEFAULTS.theme_preset;
